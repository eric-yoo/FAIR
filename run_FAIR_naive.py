...
from config import args, FAIR_NAIVE_PATH_FORMAT
...
for it in range(1, n_iters+1):
    ...
    train_res, test_res = run_simple_NN(train_xs, train_ys, test_xs, test_ys, weights,\
                                         it=it, n_epochs=args.n_epochs, mode="fair_naive")
    test_res_class      = eval_simple_NN_by_class(train_xs, train_ys, test_xs, test_ys, \
                            None, CHECKPOINTS_PATH_FORMAT.format("fair_naive", it, args.n_epochs))
    ...
    # instantiate TRACIN
    tracin = TracIn(ds_train, ds_test, \
        FAIR_NAIVE_PATH_FORMAT.format(it, args.n_epochs-2), \
        FAIR_NAIVE_PATH_FORMAT.format(it, args.n_epochs-1), \
        FAIR_NAIVE_PATH_FORMAT.format(it, args.n_epochs), \
        True)
...
df_acc.to_csv(F"test_fair_naive_accuracy_p{args.poisoned_ratio}.csv")
df_acc_cls.to_csv(F"test_fair_naive_accuracy_cls_p{args.poisoned_ratio}.csv")
df_vio.to_csv(F"test_fair_naive_violation_p{args.poisoned_ratio}.csv")
